...
<div class="bg-slate-950 min-h-screen py-8 md:py-12">
    <div class="container max-w-4xl mx-auto px-4 md:px-8 text-slate-100">
        ...
        <div class="mb-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div>
                    ...
                    <h1 class="text-3xl md:text-4xl font-bold text-white">{{ $kuis->judul }}</h1>
                </div>
                <div class="text-right mt-4 md:mt-0">
                    <p class="text-sm text-slate-400">Soal</p>
                    <p id="progress-text" class="text-2xl font-bold text-white">1 / {{ count($kuis->pertanyaan) }}</p>
                </div>
            </div>
        ...
        <!-- Form Kuis -->
        <form method="POST" action="{{ route('kuis.submit', $kuis->id) }}" id="quiz-form" class="space-y-8">
            @csrf

            @foreach($kuis->pertanyaan as $index => $pertanyaan)
                <div class="question-container bg-slate-900/80 backdrop-blur-lg border border-white/10 rounded-2xl p-8 mb-8 transition-all duration-300" data-question="{{ $index }}">
                    <!-- Question Number dan Text -->
                    <div class="mb-8">
                        <div class="inline-block mb-4">
                            <span class="px-4 py-2 bg-amber-400/20 text-amber-300 rounded-full text-sm font-semibold">
                                Pertanyaan {{ $index + 1 }} dari {{ count($kuis->pertanyaan) }}
                            </span>
                        </div>
                    ...
                    <!-- Pilihan Jawaban -->
                    <div class="space-y-3">
                        @foreach($pertanyaan['pilihan'] as $key => $pilihan)
                            <div class="relative">
                                <input 
                                    type="radio" 
                                    name="jawaban[{{ $index }}]" 
